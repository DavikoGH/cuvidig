...
import { Home, Users, LayoutGrid, MapPin, Building2, Settings, LogOut } from 'lucide-react';
  ...
  return (
    <aside className={`w-[80px] shrink-0 h-full backdrop-blur-md rounded-[20px] flex flex-col items-center py-6 border transition-all duration-500 z-20 ${
        ...
        {NAV_ITEMS.map((item) => (
          <NavLink
            ...
            className={({ isActive }) => twMerge(clsx(
              "w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200 group cursor-pointer",
              isActive 
      ...
      <div className="mt-auto pt-6 px-4 w-full flex justify-center">
        <button title="Cerrar sesión" className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-200 ${
          theme === 'night' ? 'text-slate-400 hover:bg-white/5 hover:text-white' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'
        }`}>
          <LogOut size={22} />
        </button>
